...
public class Boss3 extends GameObject {
    ...
    public Boss3(float x, float y, ObjectId id,Player player,Handler handler) {
        super(x, y, id);
        this.player = player;
    ...
    public void render(Graphics g) {
        g.drawImage(texture.boss3[0], (int)x, (int)y, null);
            if(!dying){
                if(handler.player.isDying() == false){
                if(attack){
                   handler.addObject(new Attack((int)x-60,(int) y+60, ObjectId.Attack,handler));
                   attack=false;
                }
                if(attack2){
